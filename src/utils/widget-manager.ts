...
export class WidgetManager {
	 ...
	 * 获取所有组件（无论是否启用）
	 */
	getAllComponents(): WidgetComponentConfig[] {
	 ...
	 * 获取所有启用的组件
	 */
	getEnabledComponentList(): WidgetComponentConfig[] {
...
export function getComponentConfig(
	...
	return widgetManager.getAllComponents().find((c) => c.type === componentType);
}
...
export function getEnabledComponentTypes(): WidgetComponentType[] {
	return widgetManager.getEnabledComponentList().map((c) => c.type);
}
